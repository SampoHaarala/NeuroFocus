    ...
    def _run_search(self, evaluate_candidates):
        ...
        def evaluate_candidates_with_progress(candidate_params):
            for idx, candidate in enumerate(candidates, start=1):
                print_progress(
                    f"GridSearchCV candidate {idx}/{total_candidates}: {candidate}"
                )
                candidate_start = time.time()
                evaluate_candidates([candidate])
                candidate_end = time.time()
                elapsed = candidate_end - start_time
                avg = elapsed / idx
                remaining = avg * (total_candidates - idx)
                print_progress(
                    f"Candidate {idx}/{total_candidates} done in {candidate_end - candidate_start:.1f}s; "
